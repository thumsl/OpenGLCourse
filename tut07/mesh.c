...
static void mesh_loadToVao(mesh *model, GLfloat *vertices, GLuint *indices) {
	...
	*/
	...
	glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, VERTEX_COMPONENT_COUNT * sizeof(GLfloat), (void*)(5 * sizeof(GLfloat)));
...
void mesh_draw(mesh* model, shader *S, camera *C, mat4x4 projection) {
	...
	mat4x4_identity(model->matrices.transform);
	glUniformMatrix4fv(S->locations.transformationMatrix, 1, 0, (GLfloat*)model->matrices.transform);
	glUniformMatrix4fv(S->locations.normalTransformationMatrix, 1, 0, (GLfloat*)model->matrices.translate);
